 ...
 * @author olivi
 */
public class CampaignMenu extends State {
    ...
    public CampaignMenu(PlayMenu state, GameStateManager gsm) {
        setFont(new Font("Curlz MT", Font.CENTER_BASELINE, (int) (250 / Game.widthScaler)));
        this.keys = new boolean[256];
        this.state = state;
        this.gsm = gsm;
        this.menuButtonText = new String[]{"1", "2", "3", "4", "5", "6"};
    ...
    public void createMenuButtons() {
        for (int i = 0; i < menuButtons.length; i++) {
            if (i < 3) {
                menuButtons[i] = new MenuButton((int) ((710 + (i * 200))/Game.widthScaler), (int) (400/Game.heightScaler), (int) (100/Game.widthScaler), (int) (100/Game.heightScaler), ID.MenuButton, menuButtonText[i], Assets.house);
            } else {
                menuButtons[i] = new MenuButton((int) ((710 + ((i - 3) * 200))/Game.widthScaler), (int) (600/Game.heightScaler), (int) (100/Game.widthScaler), (int) (100/Game.heightScaler), ID.MenuButton, menuButtonText[i], Assets.house);
            }
            player = GeometrywarsDA.getInstance().getPlayer(500, 400, "Clydez");
            if (player.getLevel() < i + 1) {
                menuButtons[i].setUnSelectable();
            }
        }
    ...
    private void execute() {
        gsm.setState(new CampaignState(gsm, controller, currentSelection + 1));
    }
    ...
    private void processInput() {
        ...
        //KP_DOWN
        if (keys[83]) {
            if ((currentSelection < menuButtons.length - 1) && (menuButtons[currentSelection + 1].getSelectable())){
